                    ...
                    <form action="{{ route('machines.update', $machine->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select id="station_id" name="station_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                    ...
                                    @foreach ($stations as $station)
                                        <option value="{{ $station->id }}" {{ old('station_id', $machine->station_id) == $station->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="deskripsi" id="deskripsi" value="{{ old('deskripsi', $machine->deskripsi) }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                @error('deskripsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="keterangan" class="block text-sm font-medium text-gray-700">Keterangan</label>
                                <textarea name="keterangan" id="keterangan" rows="3" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('keterangan', $machine->keterangan) }}</textarea>
                                @error('keterangan') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="foto_path" class="block text-sm font-medium text-gray-700">Ganti Foto (Opsional)</label>
                                @if($machine->foto_path)
                                <div class="mt-2 mb-2">
                                    <img src="{{ Storage::url($machine->foto_path) }}" alt="Foto Mesin" class="h-24 w-auto rounded">
