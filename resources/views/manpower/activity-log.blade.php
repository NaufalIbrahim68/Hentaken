        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                   ...
                   {{-- FORM FILTER BERDASARKAN CREATED_AT --}}
<div class="flex justify-end mb-4">
    <form action="{{ route('activity.log.manpower') }}" method="GET" class="flex items-end space-x-2">
...
<button type="submit"
        ...
        <a href="{{ route('activity.log.manpower') }}"
           class="py-4 px-6 border border-gray-300 shadow-sm text-sm font-medium rounded-md 
            ...
            Reset
                    ...
                    @if ($logs->isEmpty() && $created_date)
                        <div class="mb-4 p-4 rounded-md bg-yellow-100 border border-yellow-400 text-yellow-700">
                            Tidak ada data Henkaten Man power untuk tanggal <strong>{{ \Carbon\Carbon::parse($created_date)->format('d M Y') }}</strong>.
                        </div>
                    @endif
                    ...
                    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    <th scope="col" class="py-3 px-6">Nama Sebelum</th>
                                    <th scope="col" class="py-3 px-6">Nama Sesudah</th>
                            ...
                            <tbody>
                                @forelse ($logs as $log)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        <td class="py-4 px-6">{{ $log->nama ?? '-' }}</td>
                                        <td class="py-4 px-6">{{ $log->nama_after ?? '-' }}</td>
                                    ...
                                    @if (!$created_date)
                                        <tr class="bg-white border-b">
                                            <td colspan="7" class="py-4 px-6 text-center text-gray-500">
                                                Tidak ada data Henkaten Man Power pada tanggal tersebut.
                                            </td>
                                        </tr>
                                    @endif
                                @endforelse
