        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-end mb-4">
                    <form action="{{ route('master.log.material') }}" method="GET" class="flex items-end space-x-2">
                        <div>
                        ...
                        <a href="{{ route('master.log.material') }}" class="py-2 px-6 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-100 focus:outline-none transition">
                            Reset
                        </a>
                    </form>
                </div>
                                        ...
                                        <div class="max-w-md overflow-hidden text-xs">
                                            @php
                                                ...
                                                if ((!$stationName || !$materialName) && $log->loggable_id) {
                                                    ...
                                                    if ($material) {
                                                        $materialName = $materialName ?: $material->material_name;
                                                        $stationName = $stationName ?: ($material->station?->station_name ?? '-');
                                                        $lineArea = $lineArea ?: ($material->station?->line_area ?? '-');
                                                    }
                                                }
                                            ...
                                            @if($log->action === 'updated')
                                                <div class="mb-2 p-2 bg-gray-50 rounded border border-gray-100">
                                                    ...
                                                    <p><strong>Line Area:</strong> {{ $lineArea }}</p>
                                                </div>
                ...
                {{-- PAGINATION --}}
                <div class="p-6 pt-0">
                    {{ $logs->appends(request()->query())->links('vendor.pagination.tailwind-masterlog-material') }}
                </div>
            </div>
        </div>
    </div>
