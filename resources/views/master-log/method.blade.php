    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                                        ...
                                        <div class="max-w-md overflow-hidden text-xs">
                                            @php
                                                ...
                                                if (!$stationName && $log->loggable_id) {
                                                    $method = \App\Models\Method::with('station')->find($log->loggable_id);
                                                    $stationName = $method?->station?->station_name ?? $log->details['name'] ?? '-';
                                                }
                                                $stationName = $stationName ?: ($log->details['name'] ?? '-');
                                            @endphp
                ...
                {{-- PAGINATION --}}
                <div class="p-6 pt-0">
                    {{ $logs->appends(request()->query())->links('vendor.pagination.tailwind-masterlog-method') }}
                </div>
            </div>
        </div>
    </div>
