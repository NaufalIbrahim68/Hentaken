 ...
 <div class="w-full h-screen flex flex-col px-3 py-1">
    ...
    <form action="{{ url()->current() }}" method="GET" class="flex justify-center">
        ...
        <select name="line_area" 
            ...
            @php
                $hasLine = false; // Variabel bantu untuk melacak
            @endphp

            @foreach($lineAreas as $line)
                {{-- 
                    Kita cek apakah $line diawali dengan "FA L" atau "SMT L"
                --}}
                @if(Illuminate\Support\Str::startsWith($line, 'FA L') || Illuminate\Support\Str::startsWith($line, 'SMT L'))
                    
                    @php $hasLine = true; @endphp
            ...
            @if(!$hasLine)
                <option disabled {{ !$selectedLineArea ? 'selected' : '' }}>
                    Tidak ada Line FA L atau SMT L
                </option>
            @endif
