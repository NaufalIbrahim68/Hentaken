            ...
            <div class="w-1/3 text-center">
                <form action="{{ url()->current() }}" method="GET" class="flex justify-center">
                    ...
                    <select name="line_area" onchange="this.form.submit()"
                        ...
                        {{-- Loop untuk Delivery --}}
                        @foreach ($lineAreas as $line)
                            @if (Illuminate\Support\Str::upper($line) === 'DELIVERY')
                        ...
                        {{-- Loop untuk Incoming --}}
                        @foreach ($lineAreas as $line)
                            @if (Illuminate\Support\Str::upper($line) === 'INCOMING')
                                @php $hasIncoming = true; @endphp
                                <option value="{{ $line }}"
                                    {{ $selectedLineArea == $line ? 'selected' : '' }}>
                                    HENKATEN INCOMING
                                </option>
                            @endif
                        @endforeach

                        {{-- Fallback jika tidak ada line sama sekali --}}
                        @if (!$hasFaLine && !$hasSmtLine && !$hasDelivery && !$hasIncoming)
                            <option disabled {{ !$selectedLineArea ? 'selected' : '' }}>
                                Tidak ada Line Area tersedia
                            </option>
                        @endif
