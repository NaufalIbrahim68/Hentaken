<div class="bg-white shadow rounded p-4 flex flex-col mt-1">
    ...
    @php
        $isSmtLine = in_array($selectedLineArea ?? '', ['SMT L1', 'SMT L2']);
    @endphp

    @if ($isSmtLine)
        {{-- SMT Line: With Scroll Buttons --}}
        <div class="relative">
            {{-- Left Scroll Button --}}
            <button onclick="document.getElementById('materialTableScroll').scrollBy({left: -200, behavior: 'smooth'})"
            ...
            <div id="materialTableScroll" class="w-full overflow-x-auto scrollbar-hide scroll-smooth px-8">
                            ...
                            @foreach ($stationStatuses as $station)
                                ...
                                <td class="border border-gray-300 p-2 {{ $bgColorCell }}">
                                    <div class="flex justify-center items-center">
                                        <div class="material-status rounded-full bg-purple-600 flex items-center justify-center cursor-pointer"
                                            data-id="{{ $station['id'] }}"
                                            style="width: 32px; height: 32px; min-width: 32px; min-height: 32px;">
                                            <svg class="text-white" style="width: 16px; height: 16px;" fill="none"
    ...
    @else
        {{-- Non-SMT Line: Without Scroll Buttons --}}
        <div class="w-full">
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-gray-50">
                ...
                <div class="flex justify-center gap-3 p-2">
                    ...
                    @if ($filteredMaterialHenkatens->isNotEmpty())
                        @foreach ($filteredMaterialHenkatens as $henkaten)
                            <div class="material-card flex-shrink-0 flex flex-col space-y-1 p-1.5 rounded-lg border-2 border-shadow-500 shadow-md cursor-pointer hover:bg-gray-100 transition"
                                style="width: 220px;" onclick="showMaterialHenkatenDetail(this)"
                                ...
                                <div class="grid grid-cols-2 gap-1">
                                    <div class="bg-white shadow rounded p-1 text-center">
                                        ...
                                        <p class="text-xs font-medium py-1">
                                            {{ $henkaten->description_before ?? 'N/A' }}</p>
                                    </div>
                                    <div class="bg-white shadow rounded p-1 text-center">
                                        <h3 class="text-[8px] font-bold mb-0.5 text-red-600">NEW PART</h3>
                                        <p class="text-xs font-medium py-1">
                                            {{ $henkaten->description_after ?? 'N/A' }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-center">
                                    <div
                                        class="bg-orange-500 text-white px-1 py-0.5 rounded-full text-[7px] font-semibold">
                                        ACTIVE:
    ...
    <div id="materialHenkatenDetailModal"
        ...
        <div
            class="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden transform transition-all scale-100">


            <div
                class="sticky top-0 bg-gradient-to-r from-purple-500 to-yellow-500 px-6 py-4 flex justify-between items-center">
                    ...
                    <div class="flex items-center justify-around">
                        <div class="text-center">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" class="w-6 h-6 text-gray-500 mx-auto my-2">
                        ...
                        <div class="text-center">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" class="w-6 h-6 text-green-600 mx-auto my-2">
                                <rect x="4" y="4" width="7" height="7" rx="1" stroke-width="2" />
                                <rect x="13" y="4" width="7" height="7" rx="1" stroke-width="2" />
                                <rect x="4" y="13" width="7" height="7" rx="1" stroke-width="2" />
                                <rect x="13" y="13" width="7" height="7" rx="1" stroke-width="2" />
